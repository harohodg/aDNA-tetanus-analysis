...
then
	...
	echo "    Input options:"
	echo "    -r, --runWith        Run via \"docker\" or \"singularity\". Default: singularity"
...
# If RUNMODE was not specified, it is "Singularity".
if [ -z ${RUNMODE+x} ]
then
	RUNMODE="singularity"
else
	case "${RUNMODE}" in
		...
		*)
			echo
			echo "    Error: invalid option for -r/--runWith: $RUNMODE"
...
then
	...
	# Everything is the same as the "singularity" section above, except
	# docker is used to virtualize commands instead.
	if [ "${VARIANTS}" == "TRUE" ];
	then
		mkdir -p misc
		if [ ! -f ./misc/NC_004565.1_revComplement.fasta.bwt ];
		then
			...
			docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools faidx ./misc/NC_004565.1_revComplement.fasta
			docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools faidx ./misc/NC_004565.1_revComplement.fasta NC_004565.1:1496-5443 > ./misc/tmp.fasta

			mv ./misc/tmp.fasta ./misc/NC_004565.1_revComplement_tetX.fasta
			docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools faidx ./misc/NC_004565.1_revComplement_tetX.fasta
			docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 bwa index ./misc/NC_004565.1_revComplement.fasta
		fi
			...
			MINGOODFRAC="0."${MINGOODFRAC}""
				...
				do
					...
					do
						...
						do
							BASENAME=$(basename "${BAM}" | cut -d "_" -f 1)
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools view -bS -@ "${THREADS}" -F 4 -o tmp.bam "${BAM}"
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools sort -@ "${THREADS}" -o tmp.sort.bam tmp.bam
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools index tmp.sort.bam
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools fastq tmp.sort.bam > "${BASENAME}".fastq
							rm tmp.bam tmp.sort.bam tmp.sort.bam.bai
							docker run -it quay.io/biocontainers/bwa:0.7.17--h5bf99c6_8 bwa mem -t "${THREADS}" -R "@RG\tID:1\tSM:"${BASENAME}"\tLB:octo\tPU:illumina" ../misc/NC_004565.1_revComplement.fasta "${BASENAME}".fastq > "${BASENAME}".sam
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools view -@ "${THREADS}" -bh -o "${BASENAME}".unsorted.bam "${BASENAME}".sam
							rm "${BASENAME}".sam "${BASENAME}".fastq
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools sort -@ "${THREADS}" -o "${BASENAME}".sorted.bam "${BASENAME}".unsorted.bam
							rm "${BASENAME}".unsorted.bam
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools index "${BASENAME}".sorted.bam
							docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 samtools depth -a "${BASENAME}".sorted.bam > "${BASENAME}".depth

							docker run -it mjmansfi/octopus:0.7.4 \
								octopus \
									--threads "${THREADS}" \
									--reference ../misc/NC_004565.1_revComplement.fasta \
									--reads "${BASENAME}".sorted.bam \
								...
								docker run -it quay.io/biocontainers/bcftools:1.12--h45bccc9_1 bcftools view --output-type z --output "${BASENAME}".octopus.vcf.gz "${BASENAME}".octopus.vcf
								docker run -it quay.io/biocontainers/samtools:1.12--h9aed4be_1 htsfile "${BASENAME}".octopus.vcf.gz
								docker run -it quay.io/biocontainers/bcftools:1.12--h45bccc9_1 bcftools index "${BASENAME}".octopus.vcf.gz
								docker run -it quay.io/biocontainers/bcftools:1.12--h45bccc9_1 bcftools consensus --fasta-ref ../misc/NC_004565.1_revComplement_tetX.fasta --output "${BASENAME}".consensus.fasta "${BASENAME}".octopus.vcf.gz
								python "${SCRIPTDIR}"/mask_low_coverage_positions.py --fastaFile "${BASENAME}".consensus.fasta --depthFile "${BASENAME}".depth --region 1496-5443 --outFile "${BASENAME}".consensus.masked.fasta --noCoverageChar - --lowCoverageChar lower
						done
						cd ../
					done
					...
					docker run -it quay.io/biocontainers/bcftools:1.12--h45bccc9_1 bcftools merge --output-type v --output ./merged_variants.minPosterior_"${POSTERIOR}".minGoodFrac_"${MINGOODFRAC}".minPileupQual_"${MINPILEUP}".vcf --threads "${THREADS}" ./merged/*.vcf.gz ./singles/*.vcf.gz
					find merged singles -iname '*.vcf' | while read i; do b=$(basename $i | awk -F '.octopus' '{print $1}'); grep -v '^#' $i | while read v; do printf "$b\t$v\n"; done; done > all_variants.long.tsv
					find merged -iname '*.vcf' | while read i; do b=$(basename $i | awk -F '.octopus' '{print $1}'); grep -v '^#' $i | while read v; do printf "$b\t$v\n"; done; done > merged_variants.long.tsv
